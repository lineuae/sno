...
function buildStatusEmbed(client, cfg, guild) {
    const actionLabel = { delete: '🗑️ Suppression', warn: '⚠️ Avertissement', mute: '🔇 Mute' };
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (sub === 'on' || sub === 'off') {
            ...
            return message.channel.send(`> ${cfg.status ? '✅' : '❌'} Anti-lien **${cfg.status ? 'activé' : 'désactivé'}**.`);
        }
        ...
        if (sub === 'action') {
            ...
            if (!action || !valid.includes(action)) {
                return message.channel.send(`> \`❌\` Actions valides : \`delete\`, \`warn\`, \`mute\``);
            }
            cfg.action = action;
            await client.db.set(`antilink_${guildId}`, cfg);
        ...
        if (sub === 'mute') {
            ...
            if (ms(duration) > 2419200000) {
                return message.channel.send(`> \`❌\` La durée maximale est de 28 jours.`);
            }
        ...
        if (sub === 'whitelist') {
            ...
            if (action === 'add') {
                if (!domain) return message.channel.send(`> \`❌\` Usage : \`antilink whitelist add <domaine>\``);
                if (cfg.whitelist.includes(domain)) {
                    return message.channel.send(`> \`❌\` \`${domain}\` est déjà dans la whitelist.`);
                }
            ...
            if (action === 'remove') {
                if (!domain) return message.channel.send(`> \`❌\` Usage : \`antilink whitelist remove <domaine>\``);
                if (!cfg.whitelist.includes(domain)) {
                    return message.channel.send(`> \`❌\` \`${domain}\` n'est pas dans la whitelist.`);
                }
            ...
            return message.channel.send(`> \`❌\` Sous-commandes : \`add <domaine>\`, \`remove <domaine>\`, \`reset\`, \`list\``);
        }
        ...
        if (sub === 'exempt') {
            ...
            if (type === 'role') {
                ...
                if (!role) return message.channel.send(`> \`❌\` Mentionnez ou précisez l'ID d'un rôle.`);
            ...
            if (type === 'channel') {
                ...
                if (!channel) return message.channel.send(`> \`❌\` Mentionnez ou précisez l'ID d'un salon.`);
            ...
            return message.channel.send(`> \`❌\` Usage : \`antilink exempt role <@rôle>\` ou \`antilink exempt channel <#salon>\``);
        }

        if (sub === 'logs') {
            const arg = args[1];
            if (!arg) return message.channel.send(`> \`❌\` Usage : \`antilink logs <#salon|off>\``);
            if (arg === 'off') {
                cfg.logChannel = null;
                await client.db.set(`antilink_${guildId}`, cfg);
